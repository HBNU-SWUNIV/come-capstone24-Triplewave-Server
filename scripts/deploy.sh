...
if [ -z "$EXIST_BLUE" ]; then
  ...
  echo "blue 배포 시작 : $(date +%Y)-$(date +%m)-$(date +%d) $(date +%H):$(date +%M):$(date +%S)" | sudo tee -a "$LOG_FILE"
	...
	# docker-compose.yml 파일을 사용하여 spring-blue 프로젝트의 컨테이너를 빌드하고 실행
	sudo docker compose -p ${DOCKER_APP_NAME}-blue -f "$REPOSITORY/docker-compose.yml" up blue -d --build
  ...
  # docker-compose.yml 파일을 사용하여 spring-green 프로젝트의 컨테이너를 중지
  sudo docker compose -p ${DOCKER_APP_NAME}-green -f "$REPOSITORY/docker-compose.yml" down green
...
# blue가 실행중이면 green up
	...
	sudo docker compose -p ${DOCKER_APP_NAME}-green -f "$REPOSITORY/docker-compose.yml" up green -d --build

  sleep 30

  echo "blue 중단 시작 : $(date +%Y)-$(date +%m)-$(date +%d) $(date +%H):$(date +%M):$(date +%S)" | sudo tee -a "$LOG_FILE"
  sudo docker compose -p ${DOCKER_APP_NAME}-blue -f "$REPOSITORY/docker-compose.yml" blue down
  sudo docker image prune -af
